...
; 27 int main(int argc, char **argv) {
	ld (__a_2_main), hl
; 28     volatile int r;
; 29     r = test_cond_zero_tailcall(0);
	ld hl, 0
	call test_cond_zero_tailcall
	ld (main_r), hl
; 30     r = test_cond_zero_tailcall(5);
	ld hl, 5
	call test_cond_zero_tailcall
	ld (main_r), hl
; 31     r = test_two_cond_tailcall(1, 0);
	ld hl, 1
	ld (__a_1_test_two_cond_tailcall), hl
	ld hl, 0
	call test_two_cond_tailcall
	ld (main_r), hl
; 32     r = test_two_cond_tailcall(0, 1);
	ld hl, 0
	ld (__a_1_test_two_cond_tailcall), hl
	ld hl, 1
	call test_two_cond_tailcall
	ld (main_r), hl
; 33     r = test_two_cond_tailcall(0, 0);
	ld hl, 0
	ld (__a_1_test_two_cond_tailcall), hl
	call test_two_cond_tailcall
	ld (main_r), hl
; 34     r = test_simple_tailcall(5);
	ld hl, 5
	call test_simple_tailcall
	ld (main_r), hl
; 35     r = test_simple_tailcall(0);
	ld hl, 0
	call test_simple_tailcall
	ld (main_r), hl
; 36     return 0;
...
; 9 int test_cond_zero_tailcall(int x) {
	ld (__a_1_test_cond_zero_tailcall), hl
; 10     if (x == 0) return bar(0);
	ld a, h
	or l
	jp nz, l_0
	ld hl, 0
...
; 15 int test_two_cond_tailcall(int x, int y) {
	ld (__a_2_test_two_cond_tailcall), hl
; 16     if (x) return bar(x);
	ld hl, (__a_1_test_two_cond_tailcall)
	ld a, h
	or l
	jp nz, bar
; 17     if (y) return bar(x);
...
; 18     return 0;
	ld hl, 0
	ret
test_simple_tailcall:
; 22 int test_simple_tailcall(int x) {
	ld (__a_1_test_simple_tailcall), hl
; 23     if (x) return bar(x);
	ld a, h
	or l
	jp nz, bar
; 24     return 0;
...
__s_test_simple_tailcall equ __static_stack + 2
__a_1_test_simple_tailcall equ __s_test_simple_tailcall + 0
__s_bar equ __static_stack + 0
__a_1_bar equ __s_bar + 0
    savebin "tests\features\13\c8080.bin", __begin, __bss - __begin
