...
```bash
llvm-build/bin/clang -target i8080-unknown-v6c -nostdlib -O2 \
    main.o engine.o \
...
add the V6C LLD option `--v6c-constants-map=<path>`:

```bash
llvm-build/bin/clang -target i8080-unknown-v6c -nostdlib -O2 \
    main.o engine.o \
    -Wl,-Map,build/program.map \
    -Wl,--v6c-constants-map,build/program.constants.map \
    -o build/program.rom
```
...
```text
# V6C final global absolute constants
# Value     Symbol
