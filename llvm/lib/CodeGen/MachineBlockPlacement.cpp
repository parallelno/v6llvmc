...
MachineBlockPlacement::findBestLoopTop(const MachineLoop &L,
  ...
  if (OptForSize ||
      !F->getSubtarget().enableLoopRotationInBlockPlacement())
    return L.getHeader();
...
void MachineBlockPlacement::buildLoopChains(const MachineLoop &L) {
  ...
  // Allow targets to opt out of loop rotation entirely. On architectures
  // without an instruction cache / branch predictor / speculation (e.g.
  // i8080), the rotation always costs an extra entry JMP and more intra-loop
  // branches without any cycle saving.
  bool EnableLoopRotation =
      F->getSubtarget().enableLoopRotationInBlockPlacement();
  ...
  BlockFrequency ExitFreq;
  if (EnableLoopRotation && !RotateLoopWithProfile &&
      LoopTop == L.getHeader())
