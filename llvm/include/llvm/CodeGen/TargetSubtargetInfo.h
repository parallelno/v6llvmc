...
public:
  ...
  /// True if MachineBlockPlacement should perform loop rotation
  /// (`findBestLoopExit` + `rotateLoop` / `rotateLoopWithProfile`).
  ///
  /// Targets without an instruction cache, branch predictor, or speculative
  /// execution (e.g. i8080) gain nothing from rotating a loop so its exit
  /// edge becomes a fall-through, but pay a real cost (an extra entry JMP
  /// and a longer chain of intra-loop unconditional jumps). Returning
  /// `false` here causes block placement to leave the loop chain in its
  /// natural top-tested form, matching the layout produced under `-Os`.
  virtual bool enableLoopRotationInBlockPlacement() const { return true; }
