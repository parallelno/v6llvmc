...
void DwarfExpression::addAddress(const MCSymbol *Symbol, uint64_t Offset) {
  emitOp(dwarf::DW_OP_addrx);
  emitUnsigned(CU.getDwarfDebug().getAddressPool().getIndex(Symbol));
  if (Offset) {
    emitOp(dwarf::DW_OP_plus_uconst);
    emitUnsigned(Offset);
  }
