...
V6C debug builds use DWARF v4 metadata in the final linked ELF. When a build
...
Pass `-g -gdwarf-4` to the V6C Clang driver:
...
The companion ELF contains final symbols and standard DWARF v4 `.debug_info`,
`.debug_abbrev`, `.debug_line`, and `.debug_str` sections. It records source
file names and instruction source locations for C and supported assembly
inputs.
...
```bash
llvm-build/bin/clang -target i8080-unknown-v6c -O0 -g -gdwarf-4 \
    hello.c -o hello.rom
```
...
```bash
llvm-build/bin/clang -target i8080-unknown-v6c -O0 -g -gdwarf-4 -c main.c -o main.o
llvm-build/bin/clang -target i8080-unknown-v6c -O0 -g -gdwarf-4 -c sprite.c -o sprite.o
llvm-build/bin/clang -target i8080-unknown-v6c -g -gdwarf-4 \
...
- DWARF v4 is the supported format; use `-gdwarf-4` for portable V6C debug
  artifacts.
